...
| NIMBUS promotion evidence binder | Pass, explicitly not promotable. Checklist: 5 met, 1 partial, 3 missing. Met: session-level corpus coverage, 16 negative contexts, grouped CV, clean sealed holdout, separate FN/FP reporting. Partial: learned scaffold has an offline session-level signal, but it does not prove promotion-grade complement because sealed turn FNR is worse than deterministic beta and there is no common live head-to-head. Missing: learned runtime adapter, live learned gateway FN/FP, promoted runtime manifest. | `introspection/data/reports/aegis_nimbus_promotion_evidence_v0.json`, SHA-256 `b2ba73366b9bd1f0fae31149f0a2e1d502946706a007215bd0c7654b51ec55ba` |
...
| DP-HONEY generation-realism eval | Pass as bounded aggregate sanity evidence, not a full paper distinguisher suite: 23 registered formats, 20 scannable formats, 25 generated/reference examples per format, all generated/reference examples valid, finite entropy and likelihood metrics, raw values absent, `paper_faithful_statistical_distinguisher=false`. | `introspection/data/reports/dp_honey_generation_realism_eval_v1.json`, SHA-256 `0b57be3dcec16f316f664327e813aaf94fb532994ae5c1e895cbebe9a2870890` |
| DP-HONEY paper-faithfulness evidence report | Pass as operational beta, not promotable as paper-faithful+: 8 checklist requirements met, 1 partial, 0 missing. Met: DP-noised bigram provenance, format fidelity/generation validity, split-conformal scanner calibration, scanner FP/FN reporting, gateway substitution and ledger evidence, output leak detection, serialized tool-argument canary detection plus NIMBUS pre-dispatch accounting, redacted audit. Partial: full statistical distinguisher realism. | `introspection/data/reports/dp_honey_paper_evidence_v2.json`, SHA-256 `abe6710ae30a3f39e6e00376fa35c0c65ec5db91ef8dbd36caf9cde2b31527b6` |
...
| DP-HONEY substitution/scanner | Paper-aligned operational beta | Default smoke shows `credential_slot_status=honeytoken_substituted` on benign protected-slot flow, with canary registration feeding canary detectors and NIMBUS. Scanner evidence reports TP=500, TN=10, FP=0, FN=0 with raw values absent from the report and split-conformal confidence calibration for scanner findings. Generation-realism evidence reports 23 registered formats with all generated/reference examples valid and finite entropy/likelihood metrics, also without raw values. Generator reports include DP-noised bigram parameters, registry version, and spec hash. Tool-argument smoke blocks both raw tool payloads and planted-canary tool payloads before provider dispatch, with NIMBUS pre-dispatch leakage bits recorded. The paper evidence checklist is 8 met, 1 partial, 0 missing. | Not a full autonomous credential broker. The full paper statistical-distinguisher suite is still missing, especially numeric-substring tests and a discriminator MLP under sealed evidence, so do not claim full DP-HONEY paper-faithful+. Real honeytoken registration/substitution with external secret managers remains a production integration task. |
...
| NIMBUS | Deterministic beta plus non-promotable learned scaffold | Canary-aware deterministic critic accumulates exact, encoded, and partial leakage risk. Runtime evidence now exposes `paper_faithful_learned_critic=false` and `promotion_status=deterministic_canary_beta`. `aegis-nimbus-training-corpus` covers benign, exact canary, encoded, partial/multi-turn, paraphrased, tool-output, and delayed leak cases with split group keys. `aegis-nimbus-eval-infonce` blocks same-corpus eval unless `--allow-training-eval` is explicitly set and emits turn-level/session-level grouped-CV and sealed-holdout evidence. `aegis-nimbus-promotion-evidence` binds those artifacts to deterministic runtime smoke and records `promote_learned_runtime=false`. | Paper-faithful learned/session leakage critic is not implemented or wired into runtime. Grouped CV and sealed holdout are still small and lexical: turn FP=0, turn FN=3, turn FNR=0.2143, session FP=0, session FN=0. It still needs a larger corpus, a runtime learned adapter, live FN/FP metrics, and a promoted runtime manifest. |
...
| NIMBUS upgraded or explicitly labeled beta with plan | Met as beta plus offline scaffold | Capabilities and runtime evidence label deterministic beta; labeled deterministic eval reports TP/TN/FP/FN separately. The learned-NIMBUS calibration and sealed-holdout corpora each cover 19 records and 9 grouped sessions across 7 labels. Grouped-CV and sealed-holdout evidence report turn FP=0, turn FN=3, turn FNR=0.2143, session FP=0, session FN=0, session FNR=0.0. `aegis_nimbus_promotion_evidence_v0.json` explicitly keeps `promote_learned_runtime=false` and lists the missing learned runtime adapter, live learned gateway FN/FP, and promotion manifest. |
| DP-HONEY quantitative scanner and realism evidence exists | Met as paper-aligned beta | `dp_honey_scanner_eval_v1.json` reports TP=500, TN=10, FP=0, FN=0, FP rate=0.0, FN rate=0.0, raw values absent, and split-conformal confidence calibration with alpha=0.1. `dp_honey_generation_realism_eval_v1.json` reports 23 formats, 25 generated/reference examples per format, all generated/reference examples valid, finite entropy/likelihood metrics, and raw values absent. `dp_honey_paper_evidence_v2.json` records the paper-faithfulness checklist as 8 met, 1 partial, 0 missing, with promotion disabled until the full statistical distinguisher suite is closed. |
