...
SelectedChoiceCalibrationSourceProfile: TypeAlias = Literal[
...
_SCHEMA_VERSION = "aegis_introspection.selected_choice_calibration_manifest/v1"
...
_SOURCE_PROFILE_BUILDERS: dict[SelectedChoiceCalibrationSourceProfile, _SourceProfileBuilder] = {
...
_CALIBRATION_PROFILE_BY_SOURCE_PROFILE: dict[SelectedChoiceCalibrationSourceProfile, str] = {
    "paired_semantic_indirection_v6": "paired_semantic_indirection_v6_calibration",
    "paired_semantic_indirection_v7": "paired_semantic_indirection_v7_calibration",
}


class SelectedChoiceCalibrationError(ValueError):
    """Raised when selected-choice calibration materialization cannot proceed."""


@dataclass(frozen=True)
class SelectedChoiceCalibrationConfig:
...
@dataclass(frozen=True)
class SelectedChoiceCalibrationResult:
    manifest: JsonObject
...
def supported_selected_choice_calibration_source_profiles() -> tuple[str, ...]:
    return tuple(_SOURCE_PROFILE_BUILDERS.keys())


def materialize_selected_choice_calibration(
    config: SelectedChoiceCalibrationConfig,
    encoder: TokenOffsetEncoder,
) -> SelectedChoiceCalibrationResult:
    ...
    if len(conversion.skipped_records) > 0:
        ...
        raise SelectedChoiceCalibrationError(f"Trace conversion skipped records: {skipped_ids}.")

    selected_records = _calibration_selected_choice_records(
    ...
    return SelectedChoiceCalibrationResult(
        manifest=manifest,
        source_trace_record_count=len(trace_records),
...
def _calibration_selected_choice_records(
    ...
    for record in records:
        ...
        if record.label not in (_SAFE_LABEL, _EXFIL_LABEL):
            raise SelectedChoiceCalibrationError(f"Unsupported selected-choice label: {record.label}.")
        if record.selected_choice_char_span is None:
            raise SelectedChoiceCalibrationError(f"{record.id} missing selected_choice_char_span.")
        if record.selected_choice_token_span is None:
            raise SelectedChoiceCalibrationError(f"{record.id} missing selected_choice_token_span.")
        if record.selected_choice_readout_token_indices is None:
            raise SelectedChoiceCalibrationError(f"{record.id} missing selected_choice_readout_token_indices.")
        if len(record.selected_choice_readout_token_indices) == 0:
            raise SelectedChoiceCalibrationError(f"{record.id} selected_choice_readout_token_indices is empty.")
        if record.fallback_reason is not None:
            raise SelectedChoiceCalibrationError(f"{record.id} has fallback_reason: {record.fallback_reason}.")
        selected.append(replace(record, tags=_calibration_tags(record=record, config=config)))

    calibration_records = tuple(selected)
    if len(calibration_records) != expected_count:
        raise SelectedChoiceCalibrationError(
            f"Expected {expected_count} selected-choice rows, found {len(calibration_records)}."
        )
    _validate_balanced_rows(records=calibration_records)
    if len({record.text for record in calibration_records}) != len(calibration_records):
        raise SelectedChoiceCalibrationError("Selected-choice calibration corpus contains duplicate prompt text.")
    return calibration_records


def _calibration_tags(record: StructuredTracePromptRecord, config: SelectedChoiceCalibrationConfig) -> tuple[str, ...]:
    if SEALED_HOLDOUT_TAG in record.tags:
        raise SelectedChoiceCalibrationError(f"{record.id} cannot be materialized from sealed_holdout source.")
    tags = list(record.tags)
    calibration_profile = _calibration_profile_for_source_profile(source_profile=config.source_profile)
    for tag in (
        _CALIBRATION_TAG,
        f"corpus:{config.corpus_id}",
        f"split:{config.calibration_split_id}",
        f"profile:{calibration_profile}",
...
def _runtime_turns_from_records(
    ...
    for turn_index, record in enumerate(records, start=1):
        ...
        eval_metadata["calibration_split_id"] = config.calibration_split_id
...
def _manifest(
    ...
    return {
        ...
        "profile": _calibration_profile_for_source_profile(source_profile=config.source_profile),
...
def _validate_config(config: SelectedChoiceCalibrationConfig) -> None:
    for field_name, value in (
        ...
        if value == "":
            raise SelectedChoiceCalibrationError(f"{field_name} must not be empty.")
    if SEALED_HOLDOUT_TAG in config.calibration_split_id or "sealed" in config.calibration_split_id:
        raise SelectedChoiceCalibrationError("calibration_split_id must not identify a sealed split.")
    if config.source_profile not in _SOURCE_PROFILE_BUILDERS:
        supported = ", ".join(supported_selected_choice_calibration_source_profiles())
        raise SelectedChoiceCalibrationError(f"source_profile must be one of: {supported}.")
    if len(config.participant_ids) == 0:
        raise SelectedChoiceCalibrationError("participant_ids must not be empty.")
    for participant_id in config.participant_ids:
        if participant_id == "":
            raise SelectedChoiceCalibrationError("participant_ids must not contain empty values.")
    if config.variants_per_label < 1:
        raise SelectedChoiceCalibrationError("variants_per_label must be positive.")
    if config.readout_token_count < 1:
        raise SelectedChoiceCalibrationError("readout_token_count must be positive.")


def _ensure_output_paths(config: SelectedChoiceCalibrationConfig) -> None:
    if config.overwrite:
        return
    for field_name, path in (
        ...
        if path.exists():
            raise SelectedChoiceCalibrationError(f"{field_name} already exists: {path}.")
...
def _validate_balanced_rows(records: tuple[StructuredTracePromptRecord, ...]) -> None:
    ...
    if label_counts.get(_SAFE_LABEL) != label_counts.get(_EXFIL_LABEL):
        raise SelectedChoiceCalibrationError(f"Selected-choice label counts must balance: {label_counts}.")
    ...
    for family in {record.family for record in records}:
        ...
        if safe_count != exfil_count:
            raise SelectedChoiceCalibrationError(f"Family {family} label counts must balance.")
...
def _validate_written_structured_prompts(
    ...
    if len(examples) != expected_count:
        raise SelectedChoiceCalibrationError(f"Structured prompt count mismatch for {path}.")
    for index, example in enumerate(examples, start=1):
        if SEALED_HOLDOUT_TAG in example.tags:
            raise SelectedChoiceCalibrationError(f"Structured prompt {index} contains sealed_holdout tag.")
        if f"profile:{calibration_profile}" not in example.tags:
            raise SelectedChoiceCalibrationError(f"Structured prompt {index} missing calibration profile tag.")
        if example.selected_choice_token_span is None:
            raise SelectedChoiceCalibrationError(f"Structured prompt {index} missing selected-choice token span.")
        if example.selected_choice_readout_token_indices is None:
            raise SelectedChoiceCalibrationError(f"Structured prompt {index} missing selected-choice readout indices.")
        if example.fallback_reason is not None:
            raise SelectedChoiceCalibrationError(f"Structured prompt {index} has fallback_reason.")


def _validate_written_runtime_turns(path: Path, expected_count: int, config: SelectedChoiceCalibrationConfig) -> None:
    calibration_profile = _calibration_profile_for_source_profile(source_profile=config.source_profile)
    requests = load_runtime_requests_jsonl(path)
    if len(requests) != expected_count:
        raise SelectedChoiceCalibrationError(f"Runtime turn count mismatch for {path}.")
    for index, request in enumerate(requests, start=1):
        ...
        if not isinstance(tags, list):
            raise SelectedChoiceCalibrationError(f"Runtime request {index} tags must be a list.")
        if SEALED_HOLDOUT_TAG in tags:
            raise SelectedChoiceCalibrationError(f"Runtime request {index} contains sealed_holdout tag.")
        if f"profile:{calibration_profile}" not in tags:
            raise SelectedChoiceCalibrationError(f"Runtime request {index} missing calibration profile tag.")
        expected_window = eval_metadata.get("expected_cift_window_family")
        if expected_window != _SELECTED_CHOICE:
            raise SelectedChoiceCalibrationError(f"Runtime request {index} is not selected-choice.")
        ...
        for field_name in (
            ...
            if cift_metadata.get(field_name) is None:
                raise SelectedChoiceCalibrationError(f"Runtime request {index} missing {field_name}.")


def _source_profile_builder(source_profile: SelectedChoiceCalibrationSourceProfile) -> _SourceProfileBuilder:
...
def _calibration_profile_for_source_profile(source_profile: SelectedChoiceCalibrationSourceProfile) -> str:
    calibration_profile = _CALIBRATION_PROFILE_BY_SOURCE_PROFILE.get(source_profile)
...
def _required_json_object(record: Mapping[str, JsonValue], field_name: str, context: str) -> dict[str, JsonValue]:
    ...
    if not isinstance(value, dict):
        raise SelectedChoiceCalibrationError(f"{context}.{field_name} must be an object.")
...
def _write_runtime_turns(path: Path, turns: tuple[Mapping[str, JsonValue], ...]) -> None:
    if len(turns) == 0:
        raise SelectedChoiceCalibrationError("Cannot write an empty runtime turn dataset.")
