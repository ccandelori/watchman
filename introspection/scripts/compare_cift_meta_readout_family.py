...
from aegis_introspection.cift_meta_readout_family import (
    CiftMetaReadoutFamily,
    CiftMetaReadoutFamilyDataset,
    CiftMetaReadoutFamilyVariant,
    compare_cift_meta_readout_family,
    write_cift_meta_readout_family_json,
    write_cift_meta_readout_family_markdown,
)
...
@dataclass(frozen=True)
class CompareCiftMetaReadoutFamilyScriptConfig:
    dataset_artifacts: tuple[DatasetArtifactSpec, ...]
    output_json_path: Path
    output_markdown_path: Path
    task_name: str
    baseline_feature_key: str
    source_families: tuple[CiftMetaReadoutFamily, ...]
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run CIFT meta-head readout-family comparisons.")
    parser.add_argument(
        "--dataset-artifact",
        required=False,
        action="append",
    ...
    parser.add_argument(
        "--output-json",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_readout_family_v1.json"),
    )
    parser.add_argument(
        "--output-md",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_readout_family_v1_summary.md"),
    )
    ...
    parser.add_argument(
        ...
        required=False,
        action="append",
        choices=("full_dual_readout", "final_token_only", "mean_pool_only"),
        help="Readout/source family. Defaults to all supported families.",
    )
    parser.add_argument(
...
def _parse_source_families(values: Sequence[str] | None) -> tuple[CiftMetaReadoutFamily, ...]:
    ...
    return tuple(cast(CiftMetaReadoutFamily, value) for value in raw_values)
...
def _parse_args(argv: Sequence[str]) -> CompareCiftMetaReadoutFamilyScriptConfig:
    namespace = _build_parser().parse_args(argv)
    return CompareCiftMetaReadoutFamilyScriptConfig(
        ...
        source_families=_parse_source_families(namespace.source_family),
...
def _binary_task_config(config: CompareCiftMetaReadoutFamilyScriptConfig) -> BinaryTaskConfig:
...
def _load_datasets(config: CompareCiftMetaReadoutFamilyScriptConfig) -> tuple[CiftMetaReadoutFamilyDataset, ...]:
    return tuple(
        CiftMetaReadoutFamilyDataset(
...
def _source_feature_keys(
    dataset: CiftMetaReadoutFamilyDataset,
    ...
    if source_family == "full_dual_readout":
        return final_token_keys + mean_pool_keys
    if source_family == "final_token_only":
        return final_token_keys
    if source_family == "mean_pool_only":
...
def _variants(
    ...
    config: CompareCiftMetaReadoutFamilyScriptConfig,
) -> tuple[CiftMetaReadoutFamilyVariant, ...]:
    if len(datasets) == 0:
        raise ValueError("At least one dataset is required to resolve CIFT readout-family source features.")
    reference_dataset = datasets[0]
    return tuple(
        CiftMetaReadoutFamilyVariant(
            variant_id=source_family,
            feature_name=f"cift_meta_readout_family_{source_family}",
            source_family=source_family,
            source_feature_keys=_source_feature_keys(reference_dataset, source_family),
        ...
        for source_family in config.source_families
    )


def run_comparison(config: CompareCiftMetaReadoutFamilyScriptConfig) -> None:
    datasets = _load_datasets(config)
    variants = _variants(datasets=datasets, config=config)
    report = compare_cift_meta_readout_family(
        datasets=datasets,
    ...
    write_cift_meta_readout_family_json(config.output_json_path, report)
    write_cift_meta_readout_family_markdown(config.output_markdown_path, report)

    print(f"Wrote CIFT meta-head readout-family report to {config.output_json_path}")
    print(f"Wrote CIFT meta-head readout-family summary to {config.output_markdown_path}")
    print(
