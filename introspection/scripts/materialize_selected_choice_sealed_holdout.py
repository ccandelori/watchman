...
from aegis_introspection.cift_selected_choice_sealed_holdout import (  # noqa: E402
    SelectedChoiceSealedHoldoutConfig,
    SelectedChoiceSealedHoldoutError,
    SelectedChoiceSealedHoldoutSourceProfile,
    materialize_selected_choice_sealed_holdout,
    supported_selected_choice_sealed_holdout_source_profiles,
)
...
class MaterializeSelectedChoiceSealedHoldoutCliConfig:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Materialize a paired-semantic selected-choice sealed CIFT holdout.")
    ...
    parser.add_argument(
        "--source-profile", required=True, choices=supported_selected_choice_sealed_holdout_source_profiles()
    )
...
def _parse_args(argv: Sequence[str]) -> MaterializeSelectedChoiceSealedHoldoutCliConfig:
    namespace = _build_parser().parse_args(argv)
    participant_ids = tuple(str(value) for value in namespace.participant_id)
    return MaterializeSelectedChoiceSealedHoldoutCliConfig(
        ...
        source_profile=cast(SelectedChoiceSealedHoldoutSourceProfile, str(namespace.source_profile)),
...
def _materializer_config(
    config: MaterializeSelectedChoiceSealedHoldoutCliConfig,
) -> SelectedChoiceSealedHoldoutConfig:
...
def run_materializer(config: MaterializeSelectedChoiceSealedHoldoutCliConfig) -> None:
...
def main(argv: Sequence[str]) -> int:
    ...
    except SelectedChoiceSealedHoldoutError as exc:
