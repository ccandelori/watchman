...
@dataclass(frozen=True)
class CompareCiftMetaRegularizationSweepScriptConfig:
    ...
    meta_regularization_values: tuple[float, ...]
    risk_label: str
    ridge: float
    fold_count: int
...
def _default_meta_regularization_values() -> tuple[float, ...]:
    return (0.03, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a signed CIFT meta-head regularization sweep.")
    parser.add_argument(
        "--dataset-artifact",
        required=False,
        action="append",
    ...
    parser.add_argument(
        "--output-json",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_regularization_sweep_v1.json"),
    )
    parser.add_argument(
        "--output-md",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_regularization_sweep_v1_summary.md"),
    )
    ...
    parser.add_argument(
        "--meta-regularization-c",
        required=False,
        action="append",
        type=float,
        help="Meta-head logistic regularization C. Defaults to a fixed sweep around C=1.0.",
    )
...
def _parse_meta_regularization_values(values: Sequence[float] | None) -> tuple[float, ...]:
    parsed_values = tuple(float(value) for value in values) if values is not None else _default_meta_regularization_values()
    if len(parsed_values) == 0:
        raise ValueError("At least one meta regularization value is required.")
    if any(value <= 0.0 for value in parsed_values):
        raise ValueError("Meta regularization values must be greater than 0.")
    if len(set(parsed_values)) != len(parsed_values):
        raise ValueError("Meta regularization values must be unique.")
    return parsed_values


def _parse_args(argv: Sequence[str]) -> CompareCiftMetaRegularizationSweepScriptConfig:
    namespace = _build_parser().parse_args(argv)
    return CompareCiftMetaRegularizationSweepScriptConfig(
        ...
        meta_regularization_values=_parse_meta_regularization_values(namespace.meta_regularization_c),
...
def _binary_task_config(config: CompareCiftMetaRegularizationSweepScriptConfig) -> BinaryTaskConfig:
...
def _load_datasets(
    config: CompareCiftMetaRegularizationSweepScriptConfig,
...
def _source_feature_keys(dataset: CiftMetaRegularizationDataset) -> tuple[str, ...]:
    return (
...
def _variants(
    ...
    config: CompareCiftMetaRegularizationSweepScriptConfig,
) -> tuple[CiftMetaRegularizationVariant, ...]:
    if len(datasets) == 0:
        raise ValueError("At least one dataset is required to resolve CIFT regularization source features.")
    source_feature_keys = _source_feature_keys(datasets[0])
    return tuple(
        CiftMetaRegularizationVariant(
            variant_id=_regularization_id(value),
            feature_name=f"cift_meta_regularization_{_regularization_id(value)}",
            ...
            meta_regularization_c=value,
        )
        for value in config.meta_regularization_values
    )


def run_comparison(config: CompareCiftMetaRegularizationSweepScriptConfig) -> None:
    ...
    write_cift_meta_regularization_sweep_json(config.output_json_path, report)
    write_cift_meta_regularization_sweep_markdown(config.output_markdown_path, report)

    print(f"Wrote CIFT meta-head regularization sweep report to {config.output_json_path}")
    print(f"Wrote CIFT meta-head regularization sweep summary to {config.output_markdown_path}")
    print(
