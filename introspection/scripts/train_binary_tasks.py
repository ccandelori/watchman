...
class TrainBinaryTasksScriptConfig:
    artifact_path: Path
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate binary Aegis detector tasks.")
    parser.add_argument(
        "--artifact",
        required=False,
    ...
    parser.add_argument(
        "--output-json",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "binary_tasks.json"),
    )
    parser.add_argument(
        "--output-md",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "binary_tasks_summary.md"),
    )
...
def _parse_args(argv: Sequence[str]) -> TrainBinaryTasksScriptConfig:
    namespace = _build_parser().parse_args(argv)
    return TrainBinaryTasksScriptConfig(
...
def _binary_task_config(config: TrainBinaryTasksScriptConfig) -> BinaryTaskConfig:
...
def run_training(config: TrainBinaryTasksScriptConfig) -> None:
    artifact = load_activation_artifact(config.artifact_path)
    report = evaluate_binary_tasks(artifact, _binary_task_config(config))
    ...
    print(f"Wrote binary task report to {config.output_json_path}")
    print(f"Wrote binary task summary to {config.output_markdown_path}")
