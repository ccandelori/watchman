...
from aegis.core.contracts import JsonValue, Message, SensitiveSpan, ToolCall
...
@dataclass(frozen=True)
class HoneytokenToolInjectionResult:
...
def inject_honeytokens_into_tool_calls(
    tool_calls: tuple[ToolCall, ...],
    ledger: HoneytokenLedger,
    turn_index: int,
) -> HoneytokenToolInjectionResult:
    if turn_index < 0:
        raise HoneytokenLedgerError("turn_index must be non-negative.")
    transformed_tool_calls: list[ToolCall] = []
    sensitive_spans: list[SensitiveSpan] = []
    seen_span_keys: set[tuple[str, str, str, int | None, int | None]] = set()

    for tool_call in tool_calls:
        transformed_arguments, argument_spans = _replace_tool_argument_placeholders(
        ...
        if not isinstance(transformed_arguments, dict):
            raise HoneytokenLedgerError("transformed tool arguments must remain an object.")
        transformed_tool_calls.append(ToolCall(name=tool_call.name, arguments=transformed_arguments))
        for span in argument_spans:
            span_key = (
                span.identifier or "",
                str(span.metadata.get("tool_call_name", "")),
                str(span.metadata.get("argument_path", "")),
                span.char_start,
                span.char_end,
            )
            if span_key in seen_span_keys:
    ...
    return HoneytokenToolInjectionResult(
        tool_calls=tuple(transformed_tool_calls),
        sensitive_spans=tuple(sensitive_spans),
        canary_records=ledger.canary_records(),
    )
...
def _replace_tool_argument_placeholders(
    value: JsonValue,
    ledger: HoneytokenLedger,
    turn_index: int,
    tool_call_name: str,
    argument_path: str,
) -> tuple[JsonValue, tuple[SensitiveSpan, ...]]:
    if isinstance(value, str):
        return _replace_tool_argument_string(
            content=value,
            ledger=ledger,
            turn_index=turn_index,
    ...
    if isinstance(value, list):
        ...
        for index, item in enumerate(value):
            transformed_item, item_spans = _replace_tool_argument_placeholders(
                value=item,
                ledger=ledger,
                turn_index=turn_index,
                tool_call_name=tool_call_name,
        ...
        return transformed_items, tuple(spans)
    if isinstance(value, dict):
        transformed_mapping: dict[str, JsonValue] = {}
        spans = []
        for key, item in value.items():
            ...
            spans.extend(item_spans)
        return transformed_mapping, tuple(spans)
    return value, ()


def _replace_tool_argument_string(
    content: str,
    ledger: HoneytokenLedger,
    turn_index: int,
    tool_call_name: str,
    ...
    for match in _PLACEHOLDER_PATTERN.finditer(content):
        ...
        cursor = match.end()
    parts.append(content[cursor:])
    transformed_content = "".join(parts)
    transformed_content, scrubbed_tokens = ledger.substitute_real_secrets(
        text=transformed_content,
        turn_index=turn_index,
    )
    spans.extend(
        _tool_spans_for_scrubbed_tokens(
            content=transformed_content,
            honeytokens=scrubbed_tokens,
            tool_call_name=tool_call_name,
            argument_path=argument_path,
    ...
    return transformed_content, tuple(spans)
...
def _tool_spans_for_scrubbed_tokens(
    content: str,
    honeytokens: tuple[Honeytoken, ...],
    tool_call_name: str,
    argument_path: str,
) -> tuple[SensitiveSpan, ...]:
    spans: list[SensitiveSpan] = []
...
def _tool_sensitive_span(
    ...
    span = _sensitive_span(honeytoken=honeytoken, char_start=char_start, char_end=char_end)
    metadata = dict(span.metadata)
    metadata["tool_call_name"] = tool_call_name
    metadata["argument_path"] = argument_path
    return SensitiveSpan(
        kind=span.kind,
        source=span.source,
